...
const caculateHeadings = debounce(
  (editor: Editor, setHeadings) => {
  ...
  return (
    <>
      {headings.map((heading) => {
        return (
          <div
            className={`${headingClassMap[heading.level]} px-2 py-0.5 cursor-pointer font-serif select-none`}
            onClick={() => {
              console.log(heading);
              const targets = document.querySelectorAll(
                `h${heading.level}[type="tiptap-heading"]`,
              );
